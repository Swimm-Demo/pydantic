---
title: Standard Library Type Validators in Pydantic
---
# Introduction

This document explains the main ideas behind the implementation of standard library type validators in <SwmPath>[pydantic/\_internal/\_validators.py](pydantic/_internal/_validators.py)</SwmPath>. The goal is to show why certain design choices were made and how the main logic fits together, focusing on a selection of representative functions.

We will cover:

1. Why sequence validation is handled with custom logic.
2. Why import string validation is flexible and robust.
3. Why pattern (regex) validation is split by type.
4. Why IP address/network validation is explicit per type.
5. Why numeric and decimal constraints are implemented with helpers.
6. Why defaultdict validation needs default factory inference.

# Sequence validation logic

<SwmSnippet path="/pydantic/_internal/_validators.py" line="27">

---

Sequence validation is not as simple as just checking <SwmToken path="pydantic/_internal/_validators.py" pos="32:15:15" line-data="    &quot;&quot;&quot;Validator for `Sequence` types, isinstance(v, Sequence) has already been called.&quot;&quot;&quot;">`isinstance`</SwmToken>`(`<SwmToken path="pydantic/_internal/_validators.py" pos="40:22:22" line-data="            &quot;&#39;{type_name}&#39; instances are not allowed as a Sequence value&quot;,">`value`</SwmToken>`, `<SwmToken path="pydantic/_internal/_validators.py" pos="28:6:6" line-data="    input_value: typing.Sequence[Any],">`Sequence`</SwmToken>`)`. The validator needs to avoid treating strings and bytes as sequences, since this leads to confusing behavior (<SwmToken path="pydantic/_internal/_validators.py" pos="92:18:20" line-data="    * `dotted_path` contains more than one colon (e.g., &#39;collections:abc:Mapping&#39;)">`e.g`</SwmToken>., a string being split into characters). The code also needs to reconstruct the original sequence type after validation, which is why it converts tuples to lists for validation and then back to tuples if needed. This is a workaround for the fact that the underlying validator expects a list-like structure.
...
<SwmSnippet path="/pydantic/_internal/_validators.py" line="67">

---

Import string validation allows users to specify Python objects using dotted paths (<SwmToken path="pydantic/_internal/_validators.py" pos="92:18:20" line-data="    * `dotted_path` contains more than one colon (e.g., &#39;collections:abc:Mapping&#39;)">`e.g`</SwmToken>., <SwmToken path="pydantic/_internal/_validators.py" pos="88:4:8" line-data="    * &#39;collections.abc:Mapping&#39;: &lt;class &#39;collections.abc.Mapping&#39;&gt;">`collections.abc:Mapping`</SwmToken>). The logic is designed to handle ambiguous cases (is the last part a module or an attribute?) and to provide clear errors when the import fails. This makes it flexible for user input and robust against common mistakes.
...
<SwmSnippet path="/pydantic/_internal/_validators.py" line="78">
...
<SwmSnippet path="/pydantic/_internal/_validators.py" line="129">

---

Pattern validation is split into three functions: one for either type, one for string patterns, and one for bytes patterns. This avoids subtle bugs where a pattern of the wrong type is accepted, and gives clear error messages. The helper function compiles the pattern and raises a custom error if compilation fails.
...
<SwmSnippet path="/pydantic/_internal/_validators.py" line="139">
...
<SwmSnippet path="/pydantic/_internal/_validators.py" line="153">
...
<SwmSnippet path="/pydantic/_internal/_validators.py" line="167">
...
<SwmSnippet path="/pydantic/_internal/_validators.py" line="177">

---

Each IP-related type (address, network, interface, v4/v6) gets its own validator. This is explicit and avoids confusion between types. Each validator tries to construct the correct type and raises a custom error if parsing fails. This makes error reporting clear and keeps the logic simple.
...
<SwmSnippet path="/pydantic/_internal/_validators.py" line="187">
...
<SwmSnippet path="/pydantic/_internal/_validators.py" line="197">
...
<SwmSnippet path="/pydantic/_internal/_validators.py" line="212">
...
<SwmSnippet path="/pydantic/_internal/_validators.py" line="227">
...
<SwmSnippet path="/pydantic/_internal/_validators.py" line="237">
...
# Numeric and decimal constraints

<SwmSnippet path="/pydantic/_internal/_validators.py" line="257">

---

Numeric constraints like greater-than, less-than, and multiple-of are implemented as small helpers that raise a known error if the constraint is not met. For decimals, the code computes the number of digits and decimal places, handling both normalized and <SwmToken path="pydantic/_internal/_validators.py" pos="344:13:15" line-data="    This function handles both normalized and non-normalized Decimal instances.">`non-normalized`</SwmToken> decimals. This is needed for accurate validation of fields like `condecimal`.
...
<SwmSnippet path="/pydantic/_internal/_validators.py" line="273">
...
<SwmSnippet path="/pydantic/_internal/_validators.py" line="282">
...
<SwmSnippet path="/pydantic/_internal/_validators.py" line="291">
...
<SwmSnippet path="/pydantic/_internal/_validators.py" line="300">
...
<SwmSnippet path="/pydantic/_internal/_validators.py" line="309">
...
<SwmSnippet path="/pydantic/_internal/_validators.py" line="318">
...
<SwmSnippet path="/pydantic/_internal/_validators.py" line="329">
...
<SwmSnippet path="/pydantic/_internal/_validators.py" line="341">
...
<SwmSnippet path="/pydantic/_internal/_validators.py" line="384">
...
<SwmSnippet path="/pydantic/_internal/_validators.py" line="398">
...
# Defaultdict validation and default factory inference

<SwmSnippet path="/pydantic/_internal/_validators.py" line="413">

---

When validating a <SwmToken path="pydantic/_internal/_validators.py" pos="419:6:6" line-data=") -&gt; collections.defaultdict[Any, Any]:">`defaultdict`</SwmToken>, the code needs to infer a sensible default factory based on the value type. If the type is not recognized, or is a generic type variable, the code raises an error and instructs the user to provide an explicit factory. This prevents silent bugs and makes the behavior predictable.
...
<SwmSnippet path="/pydantic/_internal/_validators.py" line="427">
...
# Validator lookup tables

<SwmSnippet path="/pydantic/_internal/_validators.py" line="495">

---

Validator lookup tables map constraint names or types to their corresponding validator functions. This keeps the main validation logic simple and makes it easy to add or change validators in one place.
