---
title: The GenerateSchema class
---
# What is <SwmToken path="pydantic/_internal/_generate_schema.py" pos="359:5:5" line-data="            &#39;Subclassing `GenerateSchema` is not supported. The API is highly subject to change in minor versions.&#39;,">`GenerateSchema`</SwmToken>

This document covers the <SwmToken path="pydantic/_internal/_generate_schema.py" pos="359:5:5" line-data="            &#39;Subclassing `GenerateSchema` is not supported. The API is highly subject to change in minor versions.&#39;,">`GenerateSchema`</SwmToken> class, focusing on:

1. What <SwmToken path="pydantic/_internal/_generate_schema.py" pos="359:5:5" line-data="            &#39;Subclassing `GenerateSchema` is not supported. The API is highly subject to change in minor versions.&#39;,">`GenerateSchema`</SwmToken> is and its role in Pydantic.
2. All variables and functions defined in <SwmToken path="pydantic/_internal/_generate_schema.py" pos="359:5:5" line-data="            &#39;Subclassing `GenerateSchema` is not supported. The API is highly subject to change in minor versions.&#39;,">`GenerateSchema`</SwmToken>, with code citations for each.

# What is <SwmToken path="pydantic/_internal/_generate_schema.py" pos="359:5:5" line-data="            &#39;Subclassing `GenerateSchema` is not supported. The API is highly subject to change in minor versions.&#39;,">`GenerateSchema`</SwmToken>

<SwmToken path="pydantic/_internal/_generate_schema.py" pos="359:5:5" line-data="            &#39;Subclassing `GenerateSchema` is not supported. The API is highly subject to change in minor versions.&#39;,">`GenerateSchema`</SwmToken> is a core internal class in Pydantic responsible for generating the core schema representations for Python types, including Pydantic models, dataclasses, and standard types like str and datetime. It is designed to traverse Python type annotations and produce a schema that can be used for validation, serialization, and further schema generation. This class is not intended to be subclassed or extended by users, as its API is considered unstable and subject to change.

<SwmSnippet path="/pydantic/_internal/_generate_schema.py" line="333">

---

The class uses <SwmToken path="pydantic/_internal/_generate_schema.py" pos="333:1:1" line-data="    __slots__ = (">`__slots__`</SwmToken> to define its internal attributes, which include stacks and helpers for managing configuration, namespaces, type variables, and schema definitions.
...
<SwmSnippet path="/pydantic/_internal/_generate_schema.py" line="342">

---

The constructor (<SwmToken path="pydantic/_internal/_generate_schema.py" pos="342:3:3" line-data="    def __init__(">`__init__`</SwmToken>) initializes the configuration stack, namespace resolver, type variable map, and helper stacks for field and model types, as well as the definitions tracker.
...
<SwmSnippet path="/pydantic/_internal/_generate_schema.py" line="356">

---

The <SwmToken path="pydantic/_internal/_generate_schema.py" pos="356:3:3" line-data="    def __init_subclass__(cls) -&gt; None:">`__init_subclass__`</SwmToken> method disables subclassing of <SwmToken path="pydantic/_internal/_generate_schema.py" pos="359:5:5" line-data="            &#39;Subclassing `GenerateSchema` is not supported. The API is highly subject to change in minor versions.&#39;,">`GenerateSchema`</SwmToken> and emits a warning if attempted.
...
<SwmSnippet path="/pydantic/_internal/_generate_schema.py" line="364">

---

The <SwmToken path="pydantic/_internal/_generate_schema.py" pos="365:3:3" line-data="    def _config_wrapper(self) -&gt; ConfigWrapper:">`_config_wrapper`</SwmToken> property provides access to the current configuration wrapper from the stack.
...
<SwmSnippet path="/pydantic/_internal/_generate_schema.py" line="368">

---

The <SwmToken path="pydantic/_internal/_generate_schema.py" pos="369:3:3" line-data="    def _types_namespace(self) -&gt; NamespacesTuple:">`_types_namespace`</SwmToken> property returns the current types namespace from the namespace resolver.
...
<SwmSnippet path="/pydantic/_internal/_generate_schema.py" line="372">

---

The <SwmToken path="pydantic/_internal/_generate_schema.py" pos="373:3:3" line-data="    def _arbitrary_types(self) -&gt; bool:">`_arbitrary_types`</SwmToken> property checks if arbitrary types are allowed in the current configuration.
...
<SwmSnippet path="/pydantic/_internal/_generate_schema.py" line="376">

---

The class defines a large set of private methods (prefixed with `_`) for generating schemas for various Python types and constructs, such as lists, dicts, sets, enums, IP addresses, paths, deques, mappings, fractions, arbitrary types, unknown types, unions, type aliases, literals, TypedDicts, NamedTuples, tuples, types, zoneinfo, and more. Each method is responsible for handling the schema generation logic for its respective type.
...
The <SwmToken path="pydantic/_internal/_generate_schema.py" pos="697:3:3" line-data="    def generate_schema(">`generate_schema`</SwmToken> method is the main entry point for generating a core schema for a given object. It attempts to use custom schema methods if present, falls back to internal logic, and applies any custom serialization from JSON encoders.
...
The <SwmToken path="pydantic/_internal/_generate_schema.py" pos="683:3:3" line-data="    def clean_schema(self, schema: CoreSchema) -&gt; CoreSchema:">`clean_schema`</SwmToken> method finalizes the generated schema, resolving references and applying any deferred discriminators.
...
<SwmSnippet path="/pydantic/_internal/_generate_schema.py" line="353">

---

The class uses an internal <SwmToken path="pydantic/_internal/_generate_schema.py" pos="354:7:7" line-data="        self.defs = _Definitions()">`_Definitions`</SwmToken> helper to manage schema references and definitions, which is essential for handling recursive types and avoiding infinite loops during schema generation.
...
## <SwmToken path="pydantic/_internal/_generate_schema.py" pos="359:5:5" line-data="            &#39;Subclassing `GenerateSchema` is not supported. The API is highly subject to change in minor versions.&#39;,">`GenerateSchema`</SwmToken>

<SwmToken path="pydantic/_internal/_generate_schema.py" pos="359:5:5" line-data="            &#39;Subclassing `GenerateSchema` is not supported. The API is highly subject to change in minor versions.&#39;,">`GenerateSchema`</SwmToken> is used to manage namespaces when analyzing types, particularly in scenarios where no class is present on the stack. This is useful for handling simple type annotations, such as when working with <SwmToken path="pydantic/_internal/_generate_schema.py" pos="933:24:24" line-data="                    f&#39;Mixing V1 models and V2 models (or constructs, like `TypeAdapter`) is not supported. Please upgrade `{obj.__name__}` to V2.&#39;,">`TypeAdapter`</SwmToken>.

It allows specifying default global and local namespaces through a tuple, which serves as the fallback context for type analysis. Additionally, it supports adding an optional parent namespace with the lowest priority, which can be helpful for classes defined within functions by providing additional local context.
